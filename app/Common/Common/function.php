...
define('PIC_BASE_PATH', '/home/ben/work/web/dev/torrent.zengbingo.com/Public/upload/');
define('PIC_DEFAULT', '593a5b9d52efa');
...
{
    if (!isset($old[$old_name]) OR (empty($new[$new_name]) AND $enable_empty == false)) {
        return false;
    }
    if ($old[$old_name] != $new[$new_name]) {
        $update[$new_name] = $new[$new_name];
    }
    return true;
}
...
function page2limit($total_item, $each_page_item, $page = 1)
{
    ...
    $p['page'] = $page;
    if (empty($p['page']) OR !is_numeric($p['page'])) {
        $p['page'] = 1;
    }

    $p['offset'] = ($page - 1) * $each_page_item;
    if ($p['offset'] >= $total_item) {
        $p['offset'] = $total_item - $each_page_item;
    }
    if ($p['offset'] < 0) {
        $p['offset'] = 0;
    }
